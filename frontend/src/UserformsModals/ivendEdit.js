...
const IvendEdit = ({ item, setFormEntries }) => {
  // You MUST pass formId (or _id) from item or props
 ...
 const [role, setRole] = useState("")
  ...
  useEffect(() => {
    if (item) {
      ...
      setRightsArray(item.rights || [{ item: "", access: "" }]); // <-- changed here to match backend naming
      setSelectedRoles(item.roles || []); // <-- changed here to match backend naming
    }
  }, [item]);
          ...
          <form onSubmit={handleIvendSubmit}>
            ...
            <div className="mb-3">
              ...
              >
                <label className="form-label">
                  <b>
                    ACCESS RIGHTS <i> (Managers Use Only)</i>
                  </b>
                </label>
              </div>
            ...
            {/* Removed duplicated date/actioned/reviewed inputs to avoid confusion */}
